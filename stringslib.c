...
void Shist(char* instr, int length, int* hist) {
	...
	i = 0;
	while (instr[i] != '\0' && i < length) // \0 � il fine stringa
	{
		ascii = (int)instr[i];
		hist[ascii] += 1;
...
void MOSstring(int* hist, char* mos, int* occurrences) { // Most Occuring Symbol
	...
	*mos = (char)max; // Cos� non cambio il puntatore ma solo il contenuto (� l'asterisco)
...
int DOstring(int* hist) {
	...
	while (i < HIST_BINS)
	{
		if ((i > 47 && i < 58) && (hist[i] > 0)) //Cos� accetto solo le cifre (in ASCII)
			occurrences += hist[i];
...
int AOstring(int* hist) {
	...
	while (i < HIST_BINS)
	{
		if (((i > 64 && i < 91) && (hist[i] > 0)) || ((i > 96 && i < 123) && (hist[i] > 0))) //Cos� accetto solo i caratteri (in ASCII)
			occurrences += hist[i];
